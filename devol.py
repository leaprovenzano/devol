    ...
    def run(self, dataset, num_generations: int, pop_size: int, epochs: int, fitness=None, verbose=0, metric='accuracy'):
        ...
        Args:
            ...
            epochs (int): epochs to run each search, passed to keras model.fit -currently searches are
                            curtailed if no imporovement is seen in 1 epoch
            ...
            verbose (int, optional): defaults to 0, set to 1 to view keras output during training set to 
                                    2 to view keras output and model summary before each build
